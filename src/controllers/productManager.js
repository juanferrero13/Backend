...
//Implementando modelo de persistensia con MongoDB

import ProductModel from "../models/products.model.js"

class ProductManager {

    async addProduct(title, description, price, img, thumbnail, code, stock, status, category) {
        try {
            // Verifico campos requeridos
            if (!title || !description || !price || !img || !thumbnail || !code || !stock || !status || !category) {
                // Lanza un error si alguno de los campos requeridos del producto no está completo
                throw new Error("Debes completar todos los campos")
            }
    ...
    async updateProduct(id, updateProd) {
        try {
            ...
            if (!updateProduct) {
                console.log("Producto no encoontrado")
                return null
            }
